...
import { Presentation, UploadCloud, CheckCircle, Lock } from "lucide-react";
...
export default function PPTPage() {
  ...
  useEffect(() => {
    ...
    const now = new Date();
    // Final Timeline: 17 Sept (Full Paper Announce) - 25 Sept 2026 (before Final)
    const start = new Date("2026-09-17T00:00:00");
    const end = new Date("2026-09-25T23:59:59");
    
    if (debugLevel !== "2" && (now < start || now > end)) {
      setIsLocked(true);
    }
  }, []);
  ...
  if (isLocked) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-entrance relative z-10">
        ...
        <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-300 mb-4 drop-shadow-md pb-1">Pengumpulan PPT Dikunci</h1>
        <p className="text-white/80 max-w-lg text-lg leading-relaxed font-medium drop-shadow-sm">
          Pengumpulan PPT hanya dapat dilakukan setelah pengumuman Finalis (mulai <strong className="text-white">17 September - 25 September 2026</strong>).
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-entrance relative z-10">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-300 drop-shadow-md pb-1">Pengumpulan PPT (Final)</h1>
      </div>
      ...
      {!submitted ? (
        <form onSubmit={handleSubmit} className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[1.5rem] p-8 shadow-[0_10px_30px_rgba(0,0,0,0.1)] space-y-6">
          ...
          <div className="bg-yellow-500/20 text-yellow-100 p-4 rounded-[1.2rem] text-sm mb-6 border border-yellow-400/50 shadow-inner backdrop-blur-md leading-relaxed">
            <strong className="text-white">Perhatian {user.teamName}:</strong> Selamat Anda berhasil masuk ke tahap Final! Silakan kumpulkan file presentasi (PPT) Anda.
          </div>

          <Input label="Tautan PPT (Google Drive dsb)" placeholder="Masukkan link PPT yang dapat diakses publik" variant="glass" required />

          <div className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="text-sm font-bold text-white mb-2 block drop-shadow-sm">Atau Upload File PPT <span className="text-white/50 font-normal">(Opsional)</span></label>
              <div className="border border-dashed border-white/30 bg-white/5 rounded-[1.2rem] p-8 text-center hover:bg-white/10 transition-colors cursor-pointer group shadow-sm backdrop-blur-sm">
                ...
                <p className="text-sm font-bold text-white">File Presentasi</p>
                <p className="text-xs text-white/70 mt-1">Maks. 20MB (.pptx, .pdf)</p>
                <input type="file" className="hidden" accept=".pptx,.pdf" />
              </div>
            </div>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2rem] p-12 text-center shadow-[0_20px_50px_rgba(0,0,0,0.2)] relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-300 mb-4 drop-shadow-md pb-1">PPT Berhasil Terkirim!</h2>
